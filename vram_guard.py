...
POLL_INTERVAL = 1    # Check frequency (seconds)
...
class VRAMGuard:
    ...
    def update_icon(self):
        ...
        self.icon.title = f"VRAM Guard: {self.current_temp}°C | {self.last_action}"

    def control_apps(self, action):
        pids = get_gpu_pids()
        if not pids: 
            self.last_action = "Monitoring (No GPU Load)"
            return
    ...
    def open_settings(self, *_):
        ...
        if os.path.exists(app_icon_path):
            try:
                ...
                pass # If the system doesn't support ico
        ...
        self.gui_temp.pack()
        self.gui_action = ttk.Label(root, text="Action: --", foreground="blue")
        self.gui_action.pack()

        def update_gui():
            if not self.settings_window.winfo_exists(): return
            self.gui_temp.config(text=f"{self.current_temp}°C")
            self.gui_action.config(text=self.last_action)
